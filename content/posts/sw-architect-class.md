...
### 캡슐화: 기능과 사용법만 공개하고 내부 구현은 감춤

내부를 감춘다는 점에서 블랙박스 테스트와 함께 정리했다.
...
- \- (은닉) private: 해당 클래스의 메서드를 통해서만 접근가능
...
2. 다형성: 한 가지 형태가 여러 형태로 나타날 수 있는 능력
   - 오버로딩(중복정의): 메소드 이름만 같고, 파라미터(매개변수)는 자료형이나 개수가 달라야한다. 반환형은 성립 조건이 아니라서 반환형만 다르면 오버로딩되지 않는다. 같은 클래스의 블록 안에서 사용
...
- 정적 바인딩: 호출 대상을 컴파일 시점에 결정하는 것. C++의 일반 함수가 여기에 해당한다
...
사용자 인터페이스: CUI(텍스트), CLI(cmd), GUI(그래픽), NUI(음성·제스처처럼 자연스러운 상호작용, ex. 홍채 인식)
...
- 의존 관계: 하나의 클래스가 또 다른 클래스를 짧은 시간 동안 사용(참조)
  - 느슨한 결합(연관관계는 강한 결합)
  - 상대를 매개변수·지역변수·반환값으로 잠깐 사용한다(연관관계는 멤버변수로 가진다)
- 일반화 관계: 하나의 사물이 다른 사물에 비해 더 일반적인지, 구체적인지, 공통점 가진것끼리 묶음
  - 부모자식 상속. 하위에서 상위로 공통점을 뽑아 올리면 일반화, 상위에서 하위로 나누면 구체화(특수화)
  - is a kind of 관계
...
- OCP 개방-폐쇄 원칙: 확장에는 열려있으나, 변경에는 닫혀있어야한다.
  - 파일 형식별 분기를 상속 구조로 바꿔, 새 형식은 기존 클래스를 고치지 않고 하위 클래스를 추가해 지원한다
- LSP 리스코프 치환 원칙: 상위 클래스의 객체는 언제나 자신의 하위 클래스의 객체로 교체(재정의)할 수 있어야 한다.
- ISP 인터페이스 분리 원칙: 자신이 사용하지 않는 메서드와 의존 관계를 맺으면 안된다.
  - 여러 클라이언트가 하나의 큰 인터페이스에 묶이기보다(N:1), 클라이언트별로 작은 인터페이스로 나눈다(1:N)
