...
Proxy 프록시 : 실제 객체를 대신하는 대리 객체를 두어 실제 객체에 대한 접근을 제어하는 패턴. 원격 객체 접근, 접근 권한 검사, 객체 생성 지연 등에 활용한다.
...
**Interpreter** 인터프리터 : 언어의 문법을 표현식 객체의 구조로 나타내고, 각 표현식이 문맥(Context)을 이용해 문장을 해석하도록 하는 패턴.
...
**Visitor** 비지터 : 객체 구조와 객체에 수행할 연산을 분리하는 패턴. 각 요소는 `accept(visitor)`를 통해 처리를 Visitor 객체에 위임하므로, 요소 클래스를 변경하지 않고 새로운 연산을 추가할 수 있다.

**Command** 커맨드 : 요청을 객체로 캡슐화하여 요청을 전달하는 객체와 실행하는 로직을 분리하는 패턴. 구체적인 Command는 필요한 작업을 직접 수행하거나 Receiver에 위임한다.

**Strategy** 전략 : 알고리즘 군 정의, 행위를 클래스로 캡슐화해 동적으로 상호교환 자유롭게 변환

**Memento** 메멘토 : 객체의 캡슐화를 유지하면서 특정 시점의 내부 상태를 저장하고 나중에 복원하는 패턴. 실행 취소나 상태 복구 기능에 활용한다.

**Chain of Responsibility** 책임 연쇄 : 요청을 여러 처리 객체에 순서대로 전달하고, 각 객체가 요청을 처리하거나 다음 객체로 넘기는 패턴. 요청을 보낸 객체와 실제로 처리하는 객체의 결합을 줄인다.
