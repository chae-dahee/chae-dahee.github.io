...
1.  검증 Verification 테스트 : 개발자의 시각으로 테스트. 설계서 대로 만들었는지 테스트
    - 이전 단계 산출물이 현 단계의 산출물에 반영이 되었는지
    - 요구분석명세서 내용이 설계서에 반영 → 설계서의 내용대로 코딩해야 함
    - 불완전한 개발 결과를 가졌다는 한계점
2.  확인 Validation 테스트 : 체크리스트, 요구사항대로 만들었는지 테스트
...
4.  안정성(내구성) 테스트 : 며칠 동안 부하를 주면서 안정적으로 돌아가는지 테스트 → 메모리 누수(메모리 반납하지 않음)
...
![흐름 그래프 예시: 노드 8개, 간선 9개, 안쪽 영역 R1·R2와 바깥 영역 R3](/posts/sw-implementation-test/whitebox-basepath.png)
